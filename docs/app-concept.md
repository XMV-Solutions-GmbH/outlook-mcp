...
- `ol_login_status` first calls `outlook_mcp.auth.get_token(profile)` (silent refresh-token round-trip if cached access token is stale). If a token is obtained, returns `signed_in` plus `signed_in_user_upn` — derived from the token in hand, in this order: a cache entry bound to that exact token, else the token's own `upn` / `unique_name` / `preferred_username` claim (work/school accounts, no round-trip), else one `/me?$select=userPrincipalName` call made with it (the only option for personal accounts, whose tokens are opaque). If none of the three yields an answer the field is **omitted** — never guessed and never carried over from an earlier sign-in, which is what issue #83 cost. Only when no token is obtainable does it fall through to the registry lookup.
