...
- **The harness layer refuses to run without an explicit opt-in.** It creates and permanently deletes messages in a live Microsoft 365 mailbox, and nothing about `./tests/run_tests.sh all` announced that — so any machine holding a harness token cache (a laptop, a workspace where an agent runs shell commands) reached the real mailbox from an ordinary-looking test command. Every other destructive capability here is behind an explicit consent env var (`OUTLOOK_ALLOW_DRAFTS`, `OUTLOOK_ALLOW_SEND`, `OUTLOOK_ALLOW_DELETE`); the harness was the one that was not. It now needs `OUTLOOK_HARNESS_I_KNOW_THIS_HITS_A_REAL_MAILBOX=true`, parsed strictly so a near-miss like `1` or `yes` fails closed, and CI sets it in the harness job and nowhere else. The gate runs at collection time rather than in a per-file helper, so it covers harness tests that do not exist yet — the ones whose author would have forgotten to call it.

- **The harness can no longer delete a message it did not create.** The harness suite runs unattended against a real mailbox and exercises `permanentDelete`, which skips Deleted Items and is not recoverable. "We only ever delete ids we seeded" was a property of how the tests happened to be written, not an enforced one — a refactor that deleted by search result, or an id threaded through the wrong variable, would have reached real mail. Deletion now requires one of two independent proofs of ownership: the id is in the ledger of messages this run created, or its subject carries the `[outlook-mcp-harness …]` marker read back from Graph. Unprovable ownership fails closed. `tests/unit/test_harness_delete_guard.py` pins both the behaviour and the invariant that no deletion primitive exists outside the guard, and it runs in the **unit** layer so it fails on every contributor's laptop rather than only where harness credentials exist.
- **The shared-mailbox harness tests refuse to run against the signed-in identity's own mailbox.** Pointing `OUTLOOK_HARNESS_SHARED_MAILBOX_UPN` at the harness account both destroys the tests' meaning (they assert that `/users/{upn}/` routing did *not* fall back to `/me/`) and turns them into deletes against that account's own mail. The check compares the `upn` claim of the token in use, so no mailbox address is hardcoded — this is a public repository, and an address list would both leak real addresses and go stale.
- **An opt-in flag set to `false` cannot narrow an already-consented token — now detected and reported.** The scope-gating code is correct: `resolve_scopes()` computes the gated list and that list is what goes on the wire, at the device-code request and at every refresh (`.default` is used only in service-principal mode, which the delegated path never takes). But Microsoft Entra issues every scope recorded as consented for the app registration on that resource, regardless of the narrower set requested. Verified against a live tenant: a refresh asking for exactly the six base scopes came back with `Mail.Send`, `Mail.ReadWrite.Shared` and `Group-Conversation.Read.All` in `scp`. Full finding in [`docs/proposals/0003-consented-scopes-cannot-be-narrowed.md`](docs/proposals/0003-consented-scopes-cannot-be-narrowed.md) (Accepted).
- **`ol_login_status` reports the gap** as `granted_scopes_not_requested` plus a note, so an operator sees the excess privileges instead of assuming their flags removed them. The tool-surface guarantee is unaffected: with `OUTLOOK_ALLOW_SEND=false` there is no send tool to call, whatever the token carries.

### Fixed

- **The harness preflight now runs after every credential it checks has been restored.** It sat between the two restore steps, and `HARNESS_PROFILES` is built from `steps.<id>.outputs.present` — a step that has not run yet has no outputs, so the expression evaluated to `"harness,"` and the personal credential was silently never checked while the job still reported green. Caught by reading a CI log rather than by a test; `tests/unit/test_ci_workflow.py` now pins that the preflight follows every restore step.
...
- **Tool calls carrying an argument the tool does not declare are now refused, on every tool.** The SDK derives each tool's argument model from the handler signature, and that model inherited pydantic's default `extra="ignore"` — so an invented argument was dropped before the handler ran and the call succeeded with the caller's intent silently discarded. A `body_type="html"` on `ol_email_create_draft` (the real parameters are `body` for Markdown and `body_html` for raw HTML) vanished this way; the body went through the safe-mode Markdown renderer, which strips inline HTML, and the drafts reached the user with visible `<p>` tags. Every registered tool now forbids extras, and the refusal names both the rejected argument and the parameters that do exist. The advertised JSON schema carries `additionalProperties: false`, so a well-behaved client rejects the call before it is sent.
- **CI: the harness credential can no longer die of disuse.** Microsoft Entra expires a refresh token after 90 days of *inactivity*, and the `harness` job only ran on `push` and `pull_request` — so a quiet period killed the stored credential without anyone touching the repo, and `main` went red on a dependabot bump that changed nothing relevant. The job now also runs weekly (`schedule`) and on demand (`workflow_dispatch`), which redeems the stored token with roughly twelvefold margin against the 90-day window. The fork-safety guard had to name the new events explicitly: `github.event.pull_request` is null on a scheduled run, so the existing condition would have **skipped** the job — and a skipped job reports green, which would have hidden the failure until the credential expired anyway. Pinned by `tests/unit/test_ci_workflow.py`. Rationale, and why the refreshed token is deliberately *not* written back into the repo secret, in [`docs/proposals/0004-harness-credential-keep-alive.md`](docs/proposals/0004-harness-credential-keep-alive.md). Closes [#89](https://github.com/XMV-Solutions-GmbH/outlook-mcp/issues/89).
- **A dead harness credential now fails CI with one legible message.** It used to read as `2 failed, 4 passed, 10 skipped`: the first test to call `get_token()` tripped `RefreshTokenInvalidError`, which deletes the cache entry by design, which removed the file every later test guards on — so ten of them skipped with "Harness token cache missing", a message about a fresh contributor's laptop rather than about Entra. `scripts/harness_preflight.py` now checks the credentials once, up front, and distinguishes three cases: secret absent (notice, job passes — a fork cannot supply it), credential alive (silent), credential dead (job fails with an annotated error naming how to replace it). It checks **every** profile the workflow restored, driven by which secrets were present rather than one hardcoded name — the personal-account credential (`OUTLOOK_HARNESS_PERSONAL_TOKEN_JSON`) expires by the same 90-day rule and would otherwise have died the same quiet death.
...
- **The README no longer promises more than the flags can deliver.** It previously read "the default install does not request `Mail.Send` — the consent prompt does NOT include 'this app can send mail as you'", which held for a *first* consent prompt but said nothing about a token minted after a broader consent already existed. It now leads with the guarantee that is unconditional — with `OUTLOOK_ALLOW_SEND=false` no `ol_email_send_draft` tool is registered, so nothing can send — and a new section, *What the flags do and do not control*, states the boundary and the two remedies that act on the grant rather than the request: revoke the app's consent in Entra, or use your own `OUTLOOK_CLIENT_ID`. Decision record 0003 moves to Accepted (option 1); separate app registrations per flag combination (option 2) stay available as a later stage and are deliberately not done now.

### Added

- **`tests/unit/auth/test_requested_scopes_on_the_wire.py`** — regression tests asserting that the gated scopes are absent from the `scope` field of the device-code request and of the refresh grant when their flags are off, and present when they are on. The existing `resolve_scopes()` tests covered the computation; these cover the wire.
